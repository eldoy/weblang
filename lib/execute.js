...
async function execute(ast, node, state, opt, elements) {
  ...
  var error
  var element
  var target = elements || state.html
  ...
  if (typeof state.return !== 'undefined') return
  ...
  var nextElements = element ? element.children : elements
  ...
  if (Array.isArray(it) && it.length) {
    ...
    for (var i = 0; i < it.length; i++) {
      ...
      for (var child of children) {
        await execute(ast, child, state, opt, nextElements)
      }
    }
  } else {
    for (var child of children) {
